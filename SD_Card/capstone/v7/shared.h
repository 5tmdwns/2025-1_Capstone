...
typedef struct {
    ...
    time_t          last_update_time;    // Time of last received packet

    pthread_mutex_t lock;                // Protects position and distance
    pthread_mutex_t axi_mutex;           // Protects AXI register writes

    pthread_cond_t  cond_packet_ready;   // Signals new packet arrival
    pthread_mutex_t cond_mutex;          // Mutex for condition variable
    bool            new_packet_ready;    // True when a new packet is available

    bool            timeout_flag;        // True if no packet for timeout interval
} shared_data_t;
